...
def strip_html_tags(value: str) -> str:
    ...
    This is useful when you want plain-text output only. It operates on
    the original string, not the HTML-escaped value from ``sanitize_string``.
    """
    # Basic tag stripper (does not execute any HTML, just removes tags)
    return re.sub(r"<[^>]+>", "", value)


def normalize_whitespace(value: str) -> str:
    """Collapse all whitespace in a string to single spaces and strip ends."""
...
def sanitize_url(url: str, allowed_schemes: Optional[List[str]] = None) -> str:
    """Basic URL sanitizer and validator.

    Ensures the URL has an allowed scheme (default: http/https) and a netloc.
    """
